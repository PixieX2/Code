...
while running:
    ...
    for event in pygame.event.get():
        if event.type==pygame.QUIT: running=False
            ...
            gx = int(player_x//CELL_SIZE) - SCREEN[0]//(2*CELL_SIZE) + mx//CELL_SIZE
            gy = GRID_HEIGHT - 1 - (my // CELL_SIZE)
            if event.button == 1: set_block(gx,gy,0)
            elif event.button == 3: set_block(gx,gy,inventory_slots[selected_slot])
    ...
    player_vel_x = 0.0
    if keys[pygame.K_a]: player_vel_x = -speed * dt
    if keys[pygame.K_d]: player_vel_x = speed * dt
    ...
    for x in range(int(new_x//CELL_SIZE)-1,int(new_x//CELL_SIZE)+2):
        ...
        for y in col:
            if col[y]==0: continue
            block_rect = pygame.Rect(x*CELL_SIZE, SCREEN[1]-(y+1)*CELL_SIZE, CELL_SIZE, CELL_SIZE)
            if player_rect.colliderect(block_rect):
                if player_vel_x>0: new_x = block_rect.left - player_w
                elif player_vel_x<0: new_x = block_rect.right
                player_rect.x = new_x
    player_x = new_x
    ...
    for x in range(left_col,right_col+1):
        ...
        for y, block in col.items():
            ...
            rect = pygame.Rect((x*CELL_SIZE - cam_offset), SCREEN[1]-(y+1)*CELL_SIZE, CELL_SIZE, CELL_SIZE)
    ...
    # --- UI ---
    text = font.render(f"Selected: {block_names[inventory_slots[selected_slot]]}",True,(0,0,0))
    screen.blit(text,(10,10))
    coord_text = font.render(f"Player X: {int(player_x//CELL_SIZE)}", True,(0,0,0))
    screen.blit(coord_text,(10,30))
    controls_text = font.render("Controls: A/D=Move SPACE=Jump 1-3=Select LMB/RMB=Break/Place", True,(0,0,0))
